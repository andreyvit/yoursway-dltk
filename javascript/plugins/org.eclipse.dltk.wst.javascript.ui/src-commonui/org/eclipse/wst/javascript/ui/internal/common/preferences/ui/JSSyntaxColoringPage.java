...
public final class JSSyntaxColoringPage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	void applyStyles() {
		if (fText == null || fText.isDisposed())
			return;
		IStructuredDocumentRegion documentRegion = fFirstDocumentRegion;
	...
	private String getNamedStyleAtOffset(int offset) {
		...
		IStructuredDocumentRegion documentRegion = fFirstDocumentRegion;
		while (documentRegion != null) {
			if (documentRegion.containsOffset(offset)) {
	...
	private Collection getStylePreferenceKeys() {
		if (fStylePreferenceKeys == null) {
			...
			styles.add(IStyleConstantsJavaScript.DEFAULT);
			styles.add(IStyleConstantsJavaScript.KEYWORD);
			styles.add(IStyleConstantsJavaScript.LITERAL);
			styles.add(IStyleConstantsJavaScript.COMMENT);
			styles.add(IStyleConstantsJavaScript.UNFINISHED_COMMENT);
	...
	private void initRegionContextToStyleMap() {
		fContextToStyleMap.put(IStyleConstantsJavaScript.DEFAULT, IStyleConstantsJavaScript.DEFAULT);
		fContextToStyleMap.put(IStyleConstantsJavaScript.KEYWORD, IStyleConstantsJavaScript.KEYWORD);
		fContextToStyleMap.put(IStyleConstantsJavaScript.LITERAL, IStyleConstantsJavaScript.LITERAL);
		fContextToStyleMap.put(IStyleConstantsJavaScript.COMMENT, IStyleConstantsJavaScript.COMMENT);
		fContextToStyleMap.put(IStyleConstantsJavaScript.UNFINISHED_COMMENT, IStyleConstantsJavaScript.UNFINISHED_COMMENT);
	}

	private void initStyleToDescriptionMap() {
		fStyleToDescriptionMap.put(IStyleConstantsJavaScript.DEFAULT, JSCommonUIMessages.Default_Code_UI_); //$NON-NLS-1$ = "Default Code"
		fStyleToDescriptionMap.put(IStyleConstantsJavaScript.KEYWORD, JSCommonUIMessages.Keywords_UI_); //$NON-NLS-1$ = "Keywords"
		fStyleToDescriptionMap.put(IStyleConstantsJavaScript.LITERAL, JSCommonUIMessages.Literal_Strings_UI_); //$NON-NLS-1$ = "Literal Strings"
		fStyleToDescriptionMap.put(IStyleConstantsJavaScript.COMMENT, JSCommonUIMessages.Comments_UI_); //$NON-NLS-1$ = "Comments"
		fStyleToDescriptionMap.put(IStyleConstantsJavaScript.UNFINISHED_COMMENT, JSCommonUIMessages.Unfinished_Strings_and_Comments_UI_); //$NON-NLS-1$ = "Unfinished Strings and Comments"
	}
