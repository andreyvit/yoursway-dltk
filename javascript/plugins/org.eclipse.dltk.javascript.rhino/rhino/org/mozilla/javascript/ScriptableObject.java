...
/**
 * This is the default implementation of the Scriptable interface. This class
 * provides convenient default behavior that makes it easier to define host
 * objects.
 * <p>
...
public abstract class ScriptableObject implements Scriptable, Serializable,
	 ...
	 * 
	 * This is typically the same name as the constructor. Classes extending
	 * ScriptableObject must implement this abstract method.
	 */
	...
	public void put(int index, Scriptable start, Object value) {
		...
		if (start == this && isSealed()) {
			throw Context.reportRuntimeError1("msg.modify.sealed", Integer
					.toString(index));
		}
		if ((slot.attributes & ScriptableObject.READONLY) != 0) {
			return;
	 ...
	 * 
	 * The property is specified by <code>name</code> as defined for
	 * <code>has</code>.
	 * <p>
	 * 
	 * @param name
	 ...
	 * 
	 * <p>
	 * Note that the toPrimitive conversion is a no-op for every type other than
	 * Object, for which [[DefaultValue]] is called. See ECMA 9.1.
	 * <p>
	 * 
	...
	/**
	 * Custom <tt>==</tt> operator. Must return {@link Scriptable#NOT_FOUND}
	 * if this object does not have custom equality operator for the given
	 * value, <tt>Boolean.TRUE</tt> if this object is equivalent to
	 * <tt>value</tt>, <tt>Boolean.FALSE</tt> if this object is not
	 ...
	 * <li><code>jsStaticFunction_</code> for a JavaScript function that is a
	 * property of the constructor
	 * <li><code>jsGet_</code> for a getter of a JavaScript property
	 * <li><code>jsSet_</code> for a setter of a JavaScript property
	 * <li><code>jsConstructor</code> for a JavaScript function that is the
	 * constructor
	 * </ul>
	 * <p>
	 * 
	 ...
	 * argument will be passed, along with the newly created constructor and the
	 * newly created prototype.
	 * <p>
	 * 
	 * @param scope
	 ...
	 * Define a JavaScript property.
	 * 
	 * Use this method only if you wish to define getters and setters for a
	 * given property in a ScriptableObject. To create a property without
	 * special getter or setter side effects, use
	 * <code>defineProperty(String,int)</code>.
	 * 
	 * If <code>setter</code> is null, the attribute READONLY is added to the
	 * given attributes.
	 * <p>
	 * 
	...
	public void defineProperty(String propertyName, Object delegateTo,
		...
		if (parmTypes.length != 0) {
			if (parmTypes.length != 1
				...
				throw Context.reportRuntimeError1("msg.bad.getter.parms",
						getter.toString());
			}
		} else if (delegateTo != null) {
			throw Context.reportRuntimeError1("msg.obj.getter.parms", getter
					.toString());
		}
		if (setter != null) {
			if ((delegateTo == HAS_STATIC_ACCESSORS) != (Modifier
					.isStatic(setter.getModifiers()))) {
			...
			if (parmTypes.length == 2) {
				...
				if (delegateTo == null) {
					throw Context.reportRuntimeError1("msg.setter1.parms",
							setter.toString());
				}
			} else if (parmTypes.length == 1) {
				if (delegateTo != null) {
					throw Context.reportRuntimeError1("msg.setter2.expected",
							setter.toString());
				}
			} else {
			...
			if (setterTypeTag == FunctionObject.JAVA_UNSUPPORTED_TYPE) {
				throw Context.reportRuntimeError2("msg.setter2.expected",
						setterType.getName(), setter.toString());
			}
		}
	 ...
	 * FunctionObjects are constructed from the methods found, and are added to
	 ...
	 * 
	 * For example, <code>getClassPrototype(s, "Date")</code> will first walk
	 * up the parent chain to find the outermost scope, then will search that
	 * scope for the Date constructor, and then will return Date.prototype. If
	 * any of the lookups fail, or the prototype is not a JavaScript object,
	 ...
	 * 
	 * <p>
	 * Walks the parent scope chain to find an object with a null parent scope
	 * (the global object).
	 * 
	 * @param obj
	 *            a JavaScript object
	 ...
	 * Seal this object.
	 * 
	 * A sealed object may not have properties added or removed. Once an object
	 * is sealed it may not be unsealed.
	 * 
	 * @since 1.4R3
	 */
	 ...
	 * 
	 * It is an error to attempt to add or remove properties to a sealed object.
	 * 
	 ...
	 * <code>index</code>. Note that if you wish to look for properties with
	 * numerical but non-integral indicies, you should use
	...
	/**
	 * Returns whether a named property is defined in an object or any object in
	 * its prototype chain.
	 * <p>
	 * Searches the prototype chain for a property named <code>name</code>.
	 * <p>
	...
	/**
	 * Puts an indexed property in an object or in an object in its prototype
	 * chain.
	 * <p>
	 ...
	 * <p>
	 * Searches for a property with <code>name</code> in obj or its prototype
	 ...
	 * Removes the property from an object or its prototype chain.
	 * <p>
	 * Searches for a property with <code>index</code> in obj or its prototype
	 * chain. If it is found, the object's delete method is called.
	 * 
	 * @param obj
	 *            a JavaScript object
	 ...
	 * the given scope. The method first calls
	 * {@link #getTopLevelScope(Scriptable scope)} and then searches the
	 * prototype chain of the top scope for the first object containing the
	...
	private static int getSlotPosition(Slot[] slots, String id, int index) {
		if (slots != null) {
			...
			do {
				...
				if (slot != REMOVED
						&& slot.intKey == index
						&& (slot.stringKey == id || (id != null && id
								.equals(slot.stringKey)))) {
					return i;
				}
				if (++i == slots.length)
					i = 0;
	 ...
	 * 
	 * This method must be synchronized since it is altering the hash table
	 * itself. Note that we search again for the slot to set since another
	 * thread could have added the given property or caused the table to grow
	 * while this thread was searching.
	 */
	 ...
	 * 
	 * This method must be synchronized since it is altering the hash table
	 * itself. We might be able to optimize this more, but deletes are not
	 * common.
	 */
	...
	Object[] getIds(boolean getAll) {
		...
		for (int i = 0; i < s.length; i++) {
			...
			if (getAll || (slot.attributes & DONTENUM) == 0) {
				...
				a[c++] = slot.stringKey != null ? (Object) slot.stringKey
