...
import com.xored.org.mozilla.javascript.NativeWith;
import com.xored.org.mozilla.javascript.Node;
import com.xored.org.mozilla.javascript.Ref;
...
/**
 * This abstract class describes what all XML objects (XML, XMLList) should have
 * in common.
 * 
 * @see XML
 */
abstract class XMLObjectImpl extends XMLObject {
	...
	/**
	 * ecmaHas(cx, id) calls this after resolving when id to XMLName and
	 * checking it is not Uint32 index.
	 */
	abstract boolean hasXMLProperty(XMLName name);

	/**
	 * ecmaGet(cx, id) calls this after resolving when id to XMLName and
	 * checking it is not Uint32 index.
	 */
	abstract Object getXMLProperty(XMLName name);

	/**
	 * ecmaPut(cx, id, value) calls this after resolving when id to XMLName and
	 * checking it is not Uint32 index.
	 */
	abstract void putXMLProperty(XMLName name, Object value);

	/**
	 * ecmaDelete(cx, id) calls this after resolving when id to XMLName and
	 * checking it is not Uint32 index.
	 */
	...
	/**
	 * XMLObject always compare with any value and equivalentValues never
...
// #string_id_map#
	private final static int Id_constructor = 1,

	Id_addNamespace = 2, Id_appendChild = 3, Id_attribute = 4,
			Id_attributes = 5, Id_child = 6, Id_childIndex = 7,
			Id_children = 8, Id_comments = 9, Id_contains = 10, Id_copy = 11,
			Id_descendants = 12, Id_inScopeNamespaces = 13,
			Id_insertChildAfter = 14, Id_insertChildBefore = 15,
			Id_hasOwnProperty = 16, Id_hasComplexContent = 17,
			Id_hasSimpleContent = 18, Id_length = 19, Id_localName = 20,
			Id_name = 21, Id_namespace = 22, Id_namespaceDeclarations = 23,
			Id_nodeKind = 24, Id_normalize = 25, Id_parent = 26,
			Id_prependChild = 27, Id_processingInstructions = 28,
			Id_propertyIsEnumerable = 29, Id_removeNamespace = 30,
			Id_replace = 31, Id_setChildren = 32, Id_setLocalName = 33,
			Id_setName = 34, Id_setNamespace = 35, Id_text = 36,
			Id_toString = 37, Id_toSource = 38, Id_toXMLString = 39,
			Id_valueOf = 40,
...
// #/string_id_map#
	...
	public Object execIdCall(IdFunctionObject f, Context cx, Scriptable scope,
		...
		case Id_descendants: {
			XMLName xmlName = (args.length == 0) ? XMLName.formStar() : lib
					.toXMLName(cx, args[0]);
		...
		case Id_namespace: {
			String prefix = (args.length > 0) ? ScriptRuntime.toString(args[0])
					: null;
		...
		case Id_processingInstructions: {
			XMLName xmlName = (args.length > 0) ? lib.toXMLName(cx, args[0])
					: XMLName.formStar();
			return realThis.processingInstructions(xmlName);
		}
		case Id_propertyIsEnumerable: {
			return ScriptRuntime.wrapBoolean(realThis.propertyIsEnumerable(arg(
					args, 0)));
		}
		case Id_removeNamespace: {
			Namespace ns = lib.castToNamespace(cx, arg(args, 0));
