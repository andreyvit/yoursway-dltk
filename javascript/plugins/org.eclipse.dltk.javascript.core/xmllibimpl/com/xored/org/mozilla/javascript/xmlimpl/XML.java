...
class XML extends XMLObjectImpl {
	...
	final static class XScriptAnnotation extends XmlBookmark implements
			Serializable {
	...
	final static class NamespaceDeclarations {
		...
		private void addDecl(String prefix, String ns) {
			_namespaceDecls.append((prefix.length() > 0 ? "declare namespace "
		...
		String getNextPrefix(String ns) {
			...
			_namespaceDecls.append("declare namespace " + prefix + " = " + "\""
					+ ns + "\"" + "\n");
...
// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 * @return
	 */
	static XML createTextElement(XMLLibImpl lib,
			javax.xml.namespace.QName qname, String value) {
		XScriptAnnotation anno;
	...
	static XML createFromJS(XMLLibImpl lib, Object inputObject) {
		...
		if (frag.trim().startsWith("<>")) {
			throw ScriptRuntime
					.typeError("Invalid use of XML object anonymous tags <></>.");
		}
			...
			if (defaultURI.length() > 0) {
				...
				while (!cursor.toNextToken().isEnddoc()) {
					...
					if (qname.getNamespaceURI().length() == 0) {
						...
						qname = new javax.xml.namespace.QName(defaultURI, qname
								.getLocalPart());
...
/*
 * todo need to handle namespace prefix not found in XML look for namespace type
 * in the scope change.
 * 
 * String errorMsg = "Use of undefined namespace prefix: "; String msg =
 * xe.getError().getMessage(); if (msg.startsWith(errorMsg)) { String prefix =
 * msg.substring(errorMsg.length()); }
 */
...
// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	private XmlOptions getOptions() {
		...
		if (lib.prettyPrinting) {
			...
			options.put(XmlOptions.SAVE_PRETTY_PRINT_INDENT, new Integer(
					lib.prettyIndent));
		}
	...
	/*
	 * fUseStartDoc used by child(int index) the index is at startDoc is the
	 * element at the top-level otherwise we always want to drill in.
	 */
	private boolean moveToChild(XmlCursor curs, long index,
			boolean fFirstChild, boolean fUseStartDoc) {
		if (index < 0)
			throw new IllegalArgumentException();
		...
		if (!fUseStartDoc && curs.currentTokenType().isStartdoc()) {
			...
			// todo: This assumes that we want have multiple top-level nodes.
			// Which we should be able tohave.
			curs.toFirstContentToken();
		}
		...
		if (!tt.isNone() && !tt.isEnd()) {
			while (true) {
				...
				if (tt.isText()) {
					...
					// Need to do this we want to be pointing at the text if
					// that after the end token.
					curs.toEndToken();
					curs.toNextToken();
...
// curs.toFirstContentToken();
	 ...
	 * @return
	 */
	private boolean moveSrcToDest(XmlCursor srcCurs, XmlCursor destCurs,
			boolean fDontMoveIfSame) {
		boolean fMovedSomething = true;
		TokenType tt;
		do {
			if (fDontMoveIfSame && srcCurs.isInSameDocument(destCurs)
					&& (srcCurs.comparePosition(destCurs) == 0)) {
				// If the source and destination are pointing at the same place
				// then there's nothing to move.
				fMovedSomething = false;
				break;
			}

			// todo ***TLL*** Use replaceContents (when added) and eliminate
			// children removes (see above todo).
			if (destCurs.currentTokenType().isStartdoc()) {
				destCurs.toNextToken();
			}

			// todo ***TLL*** Can Eric support notion of copy instead of me
			// copying then moving???
	...
	private XmlCursor copy(XmlCursor cursToCopy) {
		...
		if (cursToCopy.currentTokenType().isText()) {
			try {
				// Try just as a textnode, to do that we need to wrap the text
				// in a special fragment tag
				// that is not visible from the XmlCursor.
				copyCurs = XmlObject.Factory.parse(
						"<x:fragment xmlns:x=\"http://www.openuri.org/fragment\">"
								+ cursToCopy.getChars() + "</x:fragment>")
						.newCursor();
				if (!cursToCopy.toNextSibling()) {
					if (cursToCopy.currentTokenType().isText()) {
						cursToCopy.toNextToken(); // It's not an element it's
						// text so skip it.
					}
				}
			} catch (Exception ex) {
			...
			{
				if (cursToCopy.currentTokenType().isText()) {
					cursToCopy.toNextToken(); // It's not an element it's text
					// so skip it.
				}
			}
	...
	private void insertChild(XmlCursor curs, Object xmlToInsert) {
		if (xmlToInsert == null || xmlToInsert instanceof Undefined) {
			...
			XmlObject xo = XmlObject.Factory.newInstance(); // Create an empty
			// document.
				...
				if (tt.isStart()) {
					...
					if (curs.comparePosition(xmlChildCursor) == 0) {
						...
						if (addToType == APPEND_CHILD) {
							// Move the cursor to just past the end of this
							// element
							curs.toEndToken();
							curs.toNextToken();
						}
	...
	protected void removeToken(XmlCursor curs) {
		...
		// Don't delete anything move to another document so it gets orphaned
		// nicely.
	...
	protected static javax.xml.namespace.QName computeQName(Object name) {
		if (name instanceof String) {
			...
			if (fullName.startsWith("\"")) {
				...
				if (idx != -1) {
					ns = fullName.substring(1, idx - 1); // Don't include the
					// "" around the
					// namespace
	...
	private void replace(XmlCursor destCurs, XML newValue) {
		if (destCurs.isStartdoc()) {
			// Can't overwrite a whole document (user really wants to overwrite
			// the contents of).
		...
		if (srcCurs.currentTokenType().isStartdoc()) {
			// Cann't append a whole document (user really wants to append the
			// contents of).
		...
		// Re-link a new annotation to this cursor -- we just deleted the
		// previous annotation on entrance to replace.
		if (!destCurs.toPrevSibling()) {
			destCurs.toPrevToken();
		}
		destCurs.setBookmark(new XScriptAnnotation(destCurs));

		// todo would be nice if destCurs.toNextSibling went to where the next
		// token if the cursor was pointing at the last token in the stream.
		destCurs.toEndToken();
		destCurs.toNextToken();
	...
	private boolean doPut(XMLName name, XML currXMLNode, XMLObjectImpl xmlValue) {
		...
		try {
			...
			// We're done we've blown away the node because the rvalue was
			// XML...
			result = true;
		} catch (Exception ex) {
			ex.printStackTrace();
				...
				if (value instanceof XMLObjectImpl) {
					...
					if (xmlValue instanceof XML) {
						if (((XML) xmlValue).tokenType() == XmlCursor.TokenType.ATTR) {
							xmlValue = makeXmlFromString(lib, xmlName, xmlValue
									.toString());
						}
					}
					...
					if (xmlValue instanceof XMLList) {
						for (int i = 0; i < xmlValue.length(); i++) {
							...
							if (xml.tokenType() == XmlCursor.TokenType.ATTR) {
								((XMLList) xmlValue).replace(i,
										makeXmlFromString(lib, xmlName, xml
												.toString()));
							}
						}
					}
				} else {
					xmlValue = makeXmlFromString(lib, xmlName, ScriptRuntime
							.toString(value));
				}
			...
			do {
				if (tt.isStart() || tt.isAttr() || tt.isNamespace()) {
					...
					if (oldURI.equals(currQName.getNamespaceURI())) {
						curs.setName(new javax.xml.namespace.QName(newURI,
								currQName.getLocalPart()));
					}
				}
	...
	void remove() {
		...
		if (childCurs.currentTokenType().isStartdoc()) {
			...
			while (!tt.isEnd() && !tt.isEnddoc()) {
				...
				tt = childCurs.currentTokenType(); // Now see where we're
				// pointing after the delete
				// -- next token.
			}
		} else {
			removeToken(childCurs);
	...
	void setAttribute(XMLName xmlName, Object value) {
		...
		try {
			qName = new javax.xml.namespace.QName(xmlName.uri(), xmlName
					.localName());
		} catch (Exception e) {
			throw ScriptRuntime.typeError(e.getMessage());
		}
	...
	private XMLList allChildNodes(String namespace) {
		...
		XmlCursor curs = newCursor();
		TokenType tt = curs.currentTokenType();
		javax.xml.namespace.QName targetProperty = new javax.xml.namespace.QName(
				namespace, "*");
			...
			while (!tt.isEnd()) {
				if (!tt.isStart()) {
					...
					if (namespace == null
							|| namespace.length() == 0
							|| namespace.equals("*")
							|| curs.getName().getNamespaceURI().equals(
									namespace)) {
						// Add it to the list
						result.addToList(findAnnotation(curs));

						// Set target property if target name is "*",
						// Otherwise if target property does not match current,
						// then
						// set to null
						if (targetProperty != null) {
							if (targetProperty.getLocalPart().equals("*")) {
								targetProperty = curs.getName();
							} else if (!targetProperty.getLocalPart().equals(
		...
		if (tt.isContainer()) {
			...
			while (nestLevel > 0) {
				...
				if (!tt.isAttr() && !tt.isEnd() && !tt.isEnddoc()) {
					// Only try to match names for elements or processing
					// instructions.
				...
				if (tt == tokenType) {
					...
					if (!tt.isStart() && !tt.isProcinst()) {
						...
						if (qnameMatches(name, curs.getName())) {
							...
							// Set target property if target name is "*",
							// Otherwise if target property does not match
							// current, then
							// set to null
							if (targetProperty != null) {
								if (targetProperty.getLocalPart().equals("*")) {
									targetProperty = curs.getName();
								} else if (!targetProperty.getLocalPart()
										.equals(curs.getName().getLocalPart())) {
									// Not a match, unset target property
									targetProperty = null;
								}
							}
	 ...
	 * @return
	 */
	private boolean qnameMatches(XMLName template,
		...
		while (true) {
			if (tt.isText()) {
				...
				if (tt.isEnd()) {
					...
					// Hit the parent start tag so get out we're down counting
					// children.
					break;
				}
			} else if (tt.isComment() || tt.isProcinst()) {
	 ...
	 * representing the namespaces in scope for this XML object in the context
	 * of its parent.
	 * 
	...
	/**
	 * Length of an XML object is always 1, it's a list of XML objects of size
	 * 1.
	 * 
	 * @return
	 */
	...
	/**
	 * The name method returns the qualified name associated with this XML
	 * object.
	 * 
	...
	Object namespace(String prefix) {
		...
		if (prefix == null) {
			if (cursor.isStart() || cursor.isAttr()) {
				Object[] inScopeNS = NamespaceHelper.inScopeNamespaces(lib,
						cursor);
			...
			result = (uri == null) ? Undefined.instance : new Namespace(lib,
					prefix, uri);
		}
	...
	Object[] namespaceDeclarations() {
		XmlCursor cursor = newCursor();
		Object[] namespaces = NamespaceHelper
				.namespaceDeclarations(lib, cursor);
		cursor.dispose();
		return namespaces;
	}
	...
	void normalize() {
		...
		// Walk through the tokens removing empty text nodes and merging
		// adjacent text nodes.
		if (tt.isStartdoc()) {
			tt = curs.toFirstContentToken();
		}
			...
			while (!(cursor.isEnd() && depth == 0)) {
				if (cursor.isStart()) {
					...
					while (i.hasNext()) {
						...
						ns = new Namespace(lib, (String) entry.getKey(),
					...
					if (nsURI.equals(n.uri())
							&& (nsPrefix == null || nsPrefix.equals(n.prefix()))) {
					...
					while (hasNext) {
						n = NamespaceHelper
								.getNamespace(lib, cursor, inScopeNS);
						if (nsURI.equals(n.uri())
								&& (nsPrefix == null || nsPrefix.equals(n
										.prefix()))) {
							// This namespace is used
							return this;
						}
					...
					if (nsPrefix == null) {
						...
						while (i.hasNext()) {
							...
							if (entry.getValue().equals(nsURI))
								NamespaceHelper.removeNamespace(cursor,
										(String) entry.getKey());
						}
					} else if (nsURI.equals(prefixToURI.get(nsPrefix))) {
						// Remove the namespace declaration that matches
						// nsPrefix
						NamespaceHelper.removeNamespace(cursor, String
								.valueOf(nsPrefix));
					}
				}
	...
	void setLocalName(String localName) {
		...
		try {
			...
			javax.xml.namespace.QName qname = cursor.getName();
			cursor.setName(new javax.xml.namespace.QName(qname
					.getNamespaceURI(), localName, qname.getPrefix()));
		} finally {
			cursor.dispose();
		}
	}
	...
	void setName(QName qname) {
		...
		try {
			...
			if (cursor.isProcinst()) {
				...
				cursor.setName(new javax.xml.namespace.QName(qname.uri(), qname
						.localName(), prefix));
			}
		} finally {
			cursor.dispose();
	...
	void setNamespace(Namespace ns) {
		...
		try {
			...
			cursor.setName(new javax.xml.namespace.QName(ns.uri(), localName(),
					prefix));
		} finally {
			cursor.dispose();
		}
		...
		try {
			if (curs.isText()) {
				...
				if (result.endsWith(end)) {
					result = result
							.substring(0, result.length() - end.length());
				}
			} else {
				result = XML.dumpNode(curs, getOptions());
			}
	...
	boolean equivalentXml(Object target) {
		...
		if (target instanceof XML) {
			...
			if (thisTT == XmlCursor.TokenType.ATTR
