...
public final class XMLLibImpl extends XMLLib implements Serializable {
	...
	XMLName toAttributeName(Context cx, Object nameValue) {
		...
		} else if (nameValue instanceof Boolean || nameValue instanceof Number
				|| nameValue == Undefined.instance || nameValue == null) {
	 ...
	 * ScriptRuntime.lastUint32Result(cx) and return null. Otherwise return the
	 * same value as toXMLName(cx, value).
	 */
	XMLName toXMLNameOrIndex(Context cx, Object value) {
		...
		} else if (value instanceof Boolean || value == Undefined.instance
				|| value == null) {
			throw badXMLName(value);
		} else {
	...
	public Namespace constructNamespace(Context cx, Object prefixValue,
		...
		if (uri.length() == 0) {
			if (prefixValue == Undefined.instance) {
				...
				if (prefix.length() != 0) {
					throw ScriptRuntime.typeError("Illegal prefix '" + prefix
							+ "' for 'no namespace'.");
				}
			}
	...
	QName constructQName(Context cx, Object nameValue) {
		...
		if (nameValue instanceof QName) {
			QName qname = (QName) nameValue;
			result = new QName(this, qname.uri(), qname.localName(), qname
					.prefix());
		} else {
	...
	private static boolean isNCNameStartChar(int c) {
		if ((c & ~0x7F) == 0) {
			...
			return (0xC0 <= c && c <= 0xD6) || (0xD8 <= c && c <= 0xF6)
					|| (0xF8 <= c && c <= 0x2FF) || (0x370 <= c && c <= 0x37D)
					|| 0x37F <= c;
		}
		return (0x200C <= c && c <= 0x200D) || (0x2070 <= c && c <= 0x218F)
	...
	/**
	 * Escapes the reserved characters in a value of an attribute and surround
	 * it by "".
	 * 
	 * @param value
	 *            Unescaped text
