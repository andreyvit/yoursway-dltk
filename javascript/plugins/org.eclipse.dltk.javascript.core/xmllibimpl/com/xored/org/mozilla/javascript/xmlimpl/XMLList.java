...
// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	XMLList(XMLLibImpl lib, Object inputObject) {
		...
		if (inputObject == null || inputObject instanceof Undefined) {
			...
			if (!frag.endsWith("</>")) {
				throw ScriptRuntime
						.typeError("XML with anonymous tag missing end anonymous tag");
			}
...
// //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void replace(int index, XML xml) {
		if (index < length()) {
			...
			// Skip over old item we're going to replace we've already add new
			// item on above line.
	...
	void putXMLProperty(XMLName xmlName, Object value) {
		...
		if (length() > 1) {
			throw ScriptRuntime
					.typeError("Assignment to lists with more that one item is not supported");
		} else if (length() == 0) {
			...
			if (targetObject != null && targetProperty != null
				...
				XMLName name2 = XMLName.formProperty(targetProperty
						.getNamespaceURI(), targetProperty.getLocalPart());
	...
	public void put(int index, Scriptable start, Object value) {
		...
		if (value instanceof XMLObject) {
			...
			if (targetProperty == null) {
				...
				xmlValue = XML.createTextElement(lib, targetProperty, value
						.toString());
			}
		}
				...
				if (xmlValue instanceof XML) {
					...
					if (list.length() > 0) {
						...
						for (int i = 1; i < list.length(); i++) {
							xmlParent.insertChildAfter(xmlParent
									.getXmlChild(lastIndexAdded), list.item(i));
							lastIndexAdded++;
	 ...
	 */
	XML item(int index) {
		return _annos != null ? getXmlFromAnnotation(index) : XML
				.createEmptyXML(lib);
	}
	...
	XML addNamespace(Namespace ns) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The addNamespace method works only on lists containing one item");
		}
	}
	...
	XML appendChild(Object xml) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The appendChild method works only on lists containing one item");
		}
	}
	...
	int childIndex() {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The childIndex method works only on lists containing one item");
		}
	}
	...
	Object[] inScopeNamespaces() {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The inScopeNamespaces method works only on lists containing one item");
		}
	}
	...
	XML insertChildAfter(Object child, Object xml) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The insertChildAfter method works only on lists containing one item");
		}
	}
	...
	XML insertChildBefore(Object child, Object xml) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The insertChildBefore method works only on lists containing one item");
		}
	}
	...
	String localName() {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The localName method works only on lists containing one item");
		}
	}
	...
	QName name() {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The name method works only on lists containing one item");
		}
	}
	...
	Object namespace(String prefix) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The namespace method works only on lists containing one item");
		}
	}
	...
	Object[] namespaceDeclarations() {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The namespaceDeclarations method works only on lists containing one item");
		}
	}
	...
	Object nodeKind() {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The nodeKind method works only on lists containing one item");
		}
	}
	...
	Object parent() {
		...
		// If everything in the list is the sameParent then return that as the
		// parent.
		return sameParent;
	}
	...
	XML prependChild(Object xml) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The prependChild method works only on lists containing one item");
		}
	}
	...
	XML removeNamespace(Namespace ns) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The removeNamespace method works only on lists containing one item");
		}
	}
	...
	XML replace(long index, Object xml) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The replace method works only on lists containing one item");
		}
	}
	...
	XML setChildren(Object xml) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The setChildren method works only on lists containing one item");
		}
	}
	...
	void setLocalName(String localName) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The setLocalName method works only on lists containing one item");
		}
	}
	...
	void setName(QName qname) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The setName method works only on lists containing one item");
		}
	}
	...
	void setNamespace(Namespace ns) {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("The setNamespace method works only on lists containing one item");
		}
	}
	...
	org.apache.xmlbeans.XmlObject getXmlObject() {
		if (length() == 1) {
			...
			throw ScriptRuntime
					.typeError("getXmlObject method works only on lists containing one item");
		}
	}
