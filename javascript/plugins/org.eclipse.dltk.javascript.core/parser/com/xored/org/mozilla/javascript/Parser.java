...
// end of per function variables
        ...
        throws IOException
    {
        ...
        Node pn = nf.createPositionedLeaf(Token.BLOCK,ts.getCursor());

        try {
            for (;;) {
            	int pos=ts.getCursor();
            	hiddenPosition=pos;
                int tt = peekToken();
        ...
        if (this.syntaxErrorCount != 0) {
            ...
            errorReporter.runtimeError(msg, sourceURI, baseLineno,
                                             null, 0);
        }
...
//        int sourceEndOffset = ts.getCursor();
...
//                                                 sourceEndOffset);
        ...
        throws IOException, ParserException
    {
        ...
        int functionSourceStart = ts.getCursor();
        int nameStart=ts.getCursor();
        ...
        fnNode.nameStart=nameStart;
        fnNode.nameEnd=nameEnd;
        ...
        try {
            ...
            functionSourceEnd = ts.getCursor();
        ...
        if (memberExprNode != null) {
            pn = nf.createAssignment(Token.ASSIGN, memberExprNode, pn,ts.getCursor());
            if (functionType != FunctionNode.FUNCTION_EXPRESSION) {
        ...
        throws IOException, ParserException
    {
        ...
        switch(tt) {
          case Token.IF: {
            ...
            if (ifTrue.hasChildren()){
            ifTrue.getLastChild().setPosition(ts.getCursor());
            }
            else ifTrue.setPosition(ts.getCursor());
            Node ifFalse = null;
            if (matchToken(Token.ELSE)) {
                ...
                decompiler.addEOL(Token.LC);
                ifFalse = statement();
                if (ifFalse.hasChildren()){
                    ifFalse.getLastChild().setPosition(ts.getCursor());
                    }
                    else ifFalse.setPosition(ts.getCursor());                
            }
          ...
          case Token.SWITCH: {
            ...
            try {
                ...
                switchLoop: for (;;) {
                    ...
                    Node block = nf.createPositionedLeaf(Token.BLOCK,ts.getCursor());
          ...
          case Token.FOR: {
            ...
            try {
                ...
                tt = peekToken();
                if (tt == Token.SEMI) {
                    init = nf.createPositionedLeaf(Token.EMPTY,ts.getCursor());
                } else {
                    if (tt == Token.VAR) {
                ...
                if (matchToken(Token.IN)) {
                    ...
                    if (peekToken() == Token.SEMI) {
                        // no loop condition
                        cond = nf.createPositionedLeaf(Token.EMPTY,ts.getCursor());
                    } else {
                        cond = expr(false);
                    }
                    ...
                    if (peekToken() == Token.RP) {
                        incr = nf.createPositionedLeaf(Token.EMPTY,ts.getCursor());
                    } else {
                        incr = expr(false);
                    }
                }
          ...
          case Token.TRY: {
            ...
            catchblocks = nf.createPositionedLeaf(Token.BLOCK,ts.getCursor());
          ...
          case Token.WITH: {
            ...
            pn = nf.createWith(obj, body, lineno,ts.getCursor());
            return pn;
          }
          ...
          case Token.SEMI:
            consumeToken();
            pn = nf.createPositionedLeaf(Token.EMPTY,ts.getCursor());
            return pn;
        ...
        throws IOException, ParserException
    {
        ...
        //pn.setPosition(hiddenPosition);
        int tt = peekToken();
        int cursor = ts.getCursor();
        if (Token.FIRST_ASSIGN <= tt && tt <= Token.LAST_ASSIGN) {
            consumeToken();
            decompiler.addToken(tt);
            
            
			pn = nf.createAssignment(tt, pn, assignExpr(inForInit),cursor);
        }
        ...
        throws IOException, ParserException
    {
        ...
        switch(tt) {
          ...
          case Token.LC: {
            ...
            decompiler.addToken(Token.RC);
            return nf.createObjectLiteral(elems,positions);
          }
          ...
          case Token.TRUE:
            decompiler.addToken(tt);
            return nf.createPositionedLeaf(tt,ts.getCursor());
