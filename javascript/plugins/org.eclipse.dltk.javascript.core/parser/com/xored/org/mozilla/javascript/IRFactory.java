...
final class IRFactory
{
     ...
     * Leaf
     */
    Node createPositionedLeaf(int nodeType,int position)
    {
    ...
    {
        Node node = new Node(Token.VAR, lineno);
		return node;
    }
     ...
     * @param offset the starting line number of the catch clause
     */
    Node createCatch(String varName, Node catchCond, Node stmts, int offset)
    {
        if (catchCond == null) {
            catchCond = new Node(Token.EMPTY);
        }
        Node node = new Node(Token.CATCH, createName(varName),
                        catchCond, stmts, offset);
                     ...
                     boolean isForEach)
    {
        ...
        Node assign = simpleAssignment(lvalue, id,body.getPosition());
                               ...
                               Node finallyBlock, int lineno)
    {
        ...
        if (hasCatch) {
            ...
            while (cb != null) {
                ...
                catchScope.setPosition(cb.getPosition());
     ...
     * @param positions 
     */
    Node createObjectLiteral(ObjArray elems, ArrayList positions)
    {
        ...
        object.putProp(Node.DESCENDANTS_FLAG, positions);
    ...
    private Node simpleAssignment(Node left, Node right, int position)
    {
        ...
        switch (nodeType) {
          ...
          case Token.GET_REF: {
            ...
            Node node = new Node(Token.SET_REF, ref, right);
            node.setPosition(position);
			return node;
          }
        }
    ...
    Node createAssignment(int assignType, Node left, Node right, int position)
    {
        ...
        switch (assignType) {
          case Token.ASSIGN:
            return simpleAssignment(left, right,position);
...
//     }
	...
	public Node createWith(Node obj, Node body, int lineno, int cursor) {
		setRequiresActivation();
        Node result = new Node(Token.BLOCK, lineno);
        result.addChildToBack(new Node(Token.ENTERWITH, obj));
        Node bodyNode = new Node(Token.WITH, body, lineno);
        result.addChildrenToBack(bodyNode);
        Node node = new Node(Token.LEAVEWITH);
