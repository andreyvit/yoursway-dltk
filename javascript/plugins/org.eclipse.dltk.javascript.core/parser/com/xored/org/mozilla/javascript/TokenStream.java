	...
	final int getToken() throws IOException {
		...
		retry: for (;;) {
			...
			if (isDigit(c) || (c == '.' && isDigit(peekChar()))) {
				...
				if (base == 16) {
					...
					while ('0' <= c && c <= '9') {
						 ...
						 * our behavior a superset of the ECMA numeric grammar.
						 * We might not always be so permissive, so we warn
						 * about it.
						 */
						if (base == 8 && c >= '8') {
			...
			case '/':
				// is it a // comment?
				if (matchChar('/')) {
					String comment = skipLine();
					parser.decompiler.addComment(comment);
					continue retry;
				}
				if (matchChar('*')) {
