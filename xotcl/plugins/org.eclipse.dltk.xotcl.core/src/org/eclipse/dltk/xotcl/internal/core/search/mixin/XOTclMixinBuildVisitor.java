...
public class XOTclMixinBuildVisitor extends ASTVisitor {
	...
	private String getKeyFromLevels(List nodes) {
		...
		if( result.endsWith(IMixinRequestor.MIXIN_NAME_SEPARATOR ) ) {
			return result.substring(0, result.length() - IMixinRequestor.MIXIN_NAME_SEPARATOR.length() );
		}
		return result;
	}
	...
	private String getKeyFromLevelsName(List nodes, String nodeName) {
		...
		return prefix + IMixinRequestor.MIXIN_NAME_SEPARATOR + tclNameToKey(split[split.length - 1]);
	}
	...
	public boolean visit(MethodDeclaration s) throws Exception {
		...
		if (name.startsWith("::")) {
			...
			if (name.indexOf("::") == -1) {
				...
				TypeDeclaration type = TclParseUtil.findTclTypeDeclarationFrom(this.moduleDeclaration, s);
				if( type != null ) {
					List levels = TclParseUtil.findLevelsTo(this.moduleDeclaration, type);
					String[] mName = name.split("::");
					info.key = this.getKeyFromLevels(levels) + IMixinRequestor.MIXIN_NAME_SEPARATOR + mName[mName.length - 1];
				}
			}
		}
...
//		System.out.println("Report proc or instproc:" + info.key);
	...
	private void visitXOTclMethod(MethodDeclaration s) {
		...
		if( declaringXOTclType != null && declaringXOTclType instanceof TypeDeclaration ) {
			List levels = TclParseUtil.findLevelsTo(this.moduleDeclaration, declaringXOTclType);
			info.key = this.getKeyFromLevels(levels) + IMixinRequestor.MIXIN_NAME_SEPARATOR + tclNameToKey(name); 
		}
		this.requestor.reportElement(info);
//		System.out.println("Report proc or instproc:" + info.key);
	}
