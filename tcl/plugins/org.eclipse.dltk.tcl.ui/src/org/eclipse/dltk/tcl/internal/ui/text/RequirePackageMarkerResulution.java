...
final class RequirePackageMarkerResulution implements
		IMarkerResolution {
	private String pkgName;
	private IScriptProject project;

	public RequirePackageMarkerResulution(String pkgName,
...
//				protected IStatus run(IProgressMonitor monitor) {
					...
					try {
						...
						for (int i = 0; i < paths.length; i++) {
							if (!spaths.contains(paths[i])) {
								IBuildpathEntry entry = DLTKCore
										.newExtLibraryEntry(paths[i]);
