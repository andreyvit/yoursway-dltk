...
public class TclCorrectionAssistant extends QuickAssistAssistant {
...
//	 private TclQuickAssistLightBulbUpdater fLightBulbUpdater;
	 ...
	 *            the editor
	 */
	public TclCorrectionAssistant(ITextEditor editor) {
		super();
		Assert.isNotNull(editor);
		fEditor = editor;

		TclCorrectionProcessor processor = new TclCorrectionProcessor(this);
...
//		}
	...
	private static int processAnnotation(Annotation annot, Position pos,
		...
		if (isInside(invocationLocation, posBegin, posEnd)) { // covers
			...
			if (newClosestPosition != -1) {
				...
				// if (JavaCorrectionProcessor.hasCorrections(annot)) { // only
				// // jump
				// // to
				// // it
				// // if
	 ...
	 * offset.
	 * 
	 * @return <code> true</code> if the last invoked completion was called
