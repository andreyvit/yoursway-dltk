...
//	private IAddValidatorDialogRequestor fRequestor
	...
	public AddGuideDialog(Shell shell,
			GuideNode node) {
		super(shell);
...
//		fRequestor= requestor;
	...
	private IStatus validate() {
		...
		if (pattern == null || pattern.trim().length() == 0 ||uri == null || uri.trim().length() == 0) {
			...
			if (ATSGuideManager.getInstance().isDuplicateName(pattern) && (fNode == null || !pattern.equals(fNode.getPattern()))) {
...
//			fRequestor.validatorAdded(Validator);
			ATSGuideManager.getInstance().addGuide(fPattern.getText(), fURI.getText());
			try {
