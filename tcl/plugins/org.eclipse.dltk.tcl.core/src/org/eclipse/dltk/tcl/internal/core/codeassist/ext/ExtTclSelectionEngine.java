 ...
 *******************************************************************************/
package org.eclipse.dltk.tcl.internal.core.codeassist.ext;
...
public class ExtTclSelectionEngine extends ScriptSelectionEngine {
	...
	private boolean checkMethodFrom(TypeDeclaration declaringType,
	...
	private void fillSuperClassesTo(TypeDeclaration declaringType,
			List supersToHandle) {
	...
	private void findFieldFromSearch(String varName) {
		...
		SearchRequestor requestor = new SearchRequestor() {
			...
			private void processField(final String name, IField field) {
				...
				if (mn.equals(name)
						&& !ExtTclSelectionEngine.this.selectionElements
								.contains(field)) {
					ExtTclSelectionEngine.this.selectionElements.add(field);
				}
			}
		};
		...
		SearchRequestor requestor = new SearchRequestor() {
			public void acceptSearchMatch(SearchMatch match)
				...
				if (element instanceof IType) {
					...
					if (mn.equals(name)
							&& !ExtTclSelectionEngine.this.selectionElements
									.contains(type)) {
						ExtTclSelectionEngine.this.selectionElements.add(type);
					}
			...
			private void processMethod(final String name, IMethod method) {
				...
				if (mn.equals(name)
						&& !ExtTclSelectionEngine.this.selectionElements
								.contains(method)) {
					ExtTclSelectionEngine.this.selectionElements.add(method);
				}
			}
		};
		IDLTKSearchScope scope = SearchEngine.createWorkspaceScope(toolkit);
