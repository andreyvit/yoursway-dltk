...
public class ExtTclSelectionParser extends TclAssistParser {
		...
		public boolean visit(Statement s) throws Exception {
			if (s.sourceStart() <= position && s.sourceEnd() >= position) {
				if (s instanceof TclStatement) {
					...
					ExtTclSelectionParser.this.parseBlockStatements(s, inNode,
							position);
		...
		public boolean visit(Expression s) throws Exception {
			if (s.sourceStart() <= position && s.sourceEnd() >= position) {
				if (s instanceof StringLiteral) {
					ASTNode inNode = findInNode(this.module, s);
					ExtTclSelectionParser.this.processStringLiteral(s, inNode,
							position, s);
