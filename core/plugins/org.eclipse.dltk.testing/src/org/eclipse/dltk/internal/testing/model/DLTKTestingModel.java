...
//			});
		...
		public void launchChanged(final ILaunch launch) {
			...
			final IScriptProject javaProject= DLTKTestingLaunchConfigurationConstants.getScriptProject(config);
			if (javaProject == null)
				return;
			
			// test whether the launch defines the JUnit attributes
			String atr = launch.getAttribute(org.eclipse.dltk.testing.ITestKind.LAUNCH_ATTR_TEST_KIND);
			if ((atr != null)) {
//				org.eclipse.dltk.testing.ITestKind testKind = TestKindHandler.getInstance().getTestKind(atr);
//				if (testKind != null) {
					fTrackedLaunches.remove(launch);
					getDisplay().asyncExec(new Runnable() {
						public void run() {
...
//				}
		...
		private void connectTestRunner(ILaunch launch, IScriptProject javaProject) {
			...
			int maxCount= DLTKTestingPlugin.getDefault().getPreferenceStore().getInt(DLTKTestingPreferencesConstants.MAX_TEST_RUNS);
		...
		public void sessionAdded(TestRunSession testRunSession) {
			...
			fTestSessionListener= new ITestSessionListener() {
				...
				public void sessionStarted() {
					org.eclipse.dltk.testing.ITestRunListener[] testRunListeners= DLTKTestingPlugin.getDefault().getTestRunListeners();
