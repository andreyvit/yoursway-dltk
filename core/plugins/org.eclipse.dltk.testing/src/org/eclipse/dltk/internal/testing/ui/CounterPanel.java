...
public class CounterPanel extends Composite {
	...
	public CounterPanel(Composite parent) {
		...
		fNumberOfRuns= createLabel(DLTKTestingMessages.CounterPanel_label_runs, null, " 0/0  "); //$NON-NLS-1$
		fNumberOfErrors= createLabel(DLTKTestingMessages.CounterPanel_label_errors, fErrorIcon, " 0 "); //$NON-NLS-1$
		fNumberOfFailures= createLabel(DLTKTestingMessages.CounterPanel_label_failures, fFailureIcon, " 0 "); //$NON-NLS-1$
	...
	public void setRunValue(int value, int ignoredCount) {
		String runString;
		if (ignoredCount == 0)
			runString= Messages.format(DLTKTestingMessages.CounterPanel_runcount, new String[] { Integer.toString(value), Integer.toString(fTotal) });
		else
			runString= Messages.format(DLTKTestingMessages.CounterPanel_runcount_ignored, new String[] { Integer.toString(value), Integer.toString(fTotal), Integer.toString(ignoredCount) });
