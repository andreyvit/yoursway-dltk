 ...
 * when executing <code>handleScriptFolderDelete</code>. This is part of the
...
public class DeleteModifications extends RefactoringModifications {
	 ...
	 *   
	 */
	private void handleScriptFolderDelete(IScriptFolder pack) throws CoreException {		
		...
		if (!pack.isRootFolder() && canRemoveCompletely(pack)) {
			...
			for (int m= 0; m < members.length; m++) {
				...
				if (member instanceof IFile) {
					...
					if (pack.isRootFolder() && ( toolkit == null || ( toolkit != null && (toolkit.validateSourceModule(file).getSeverity() == Status.ERROR))) )
						continue;
