...
//			String result= info.getTypeContainerName();
	...
	private static abstract class AbstractSearchJob extends AbstractJob {
		...
		protected IStatus doRun(ProgressMonitor monitor) {
			try {
				...
				return new Status(IStatus.ERROR, DLTKUIPlugin.getPluginId(), IStatus.ERROR, DLTKUIMessages.TypeInfoViewer_job_error, e);
		...
		private IStatus canceled(Exception e, boolean removePendingItems) {
			...
			return new Status(IStatus.CANCEL, DLTKUIPlugin.getPluginId(), IStatus.CANCEL, DLTKUIMessages.TypeInfoViewer_job_cancel, e);
		}
		private IStatus ok() {
			return new Status(IStatus.OK, DLTKUIPlugin.getPluginId(), IStatus.OK, "", null); //$NON-NLS-1$
		}
	...
	private static class SearchEngineJob extends AbstractSearchJob {
		...
		public SearchEngineJob(int ticket, TypeInfoViewer viewer, TypeInfoFilter filter, OpenTypeHistory history, int numberOfVisibleItems, int mode, 
				IDLTKSearchScope scope, int elementKind, IDLTKUILanguageToolkit toolkit ) {
	...
	public TypeInfoViewer(Composite parent, int flags, Label progressLabel, 
			...
			ITypeInfoFilterExtension filterExtension, ITypeInfoImageProvider imageExtension, IDLTKUILanguageToolkit toolkit ) {
