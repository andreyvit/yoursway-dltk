...
import org.eclipse.dltk.internal.ui.preferences.UserLibraryPreferencePage;
...
public class UserLibraryWizardPage extends NewElementWizardPage implements IBuildpathContainerPage, IBuildpathContainerPageExtension, IBuildpathContainerPageExtension2  {
	...
	public UserLibraryWizardPage() {
		...
		fLibrarySelector= new CheckedListDialogField(adapter, buttonLabels, new BPListLabelProvider());
	...
	private List updateLibraryList() {
		...
		for (int i= 0; i < names.length; i++) {
			String curr= names[i];
			IPath path= new Path(DLTKCore.USER_LIBRARY_CONTAINER_ID).append(curr);
			try {
		...
		if (field == fLibrarySelector) {
			...
			if (fIsEditMode) {
				if (list.size() > 1) {
					...
					fEditResult= (BPUserLibraryElement) list.get(0); // take the first
	...
	/* (non-Javadoc)
	 * @see org.eclipse.jdt.ui.wizards.IBuildpathContainerPageExtension#initialize(org.eclipse.jdt.core.IJavaProject, org.eclipse.jdt.core.IBuildpathEntry[])
	 */
