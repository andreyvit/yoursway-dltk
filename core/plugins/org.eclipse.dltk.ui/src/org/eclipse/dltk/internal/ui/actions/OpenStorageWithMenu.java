...
public class OpenStorageWithMenu extends ContributionItem {
     ...
     */
    public static final String ID = PlatformUI.PLUGIN_ID + ".OpenStorageWithMenu";//$NON-NLS-1$
     ...
     * @param page the page where the editor is opened if an item within
     *		the menu is selected
     * @param storage the selected file
     */
    ...
    private ImageDescriptor getImageDescriptor(IEditorDescriptor editorDesc) {
        ...
        if (editorDesc == null) {
            imageDesc = registry
                    .getImageDescriptor(getStorage().getName());
...
//        }
    ...
    private IStorage getStorage() {
        ...
        IStorage resource = (IStorage) this.storage
    ...
    private void openEditor(IEditorDescriptor editor) {
        ...
        try {
            String editorId = editor == null ? IEditorRegistry.SYSTEM_EXTERNAL_EDITOR_ID
                    : editor.getId();
            (page).openEditor(new ExternalStorageEditorInput(storage), editorId, true, MATCH_BOTH);
     ...
     *
     * @param menu the menu to add the item to
     * @param storage the file being edited
     */
            ...
            public void handleEvent(Event event) {
                ...
                case SWT.Selection:
                    if (menuItem.getSelection()) {
                        ...
                        try {                        	                        	
                            page.openEditor(new ExternalStorageEditorInput(storage), desc.getId(), true, MATCH_BOTH);
                        } catch (PartInitException e) {
                            DLTKUIPlugin.log(e);
                        }
                    }
