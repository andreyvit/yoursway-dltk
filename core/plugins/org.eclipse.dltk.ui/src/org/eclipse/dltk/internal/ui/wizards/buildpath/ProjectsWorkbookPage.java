...
public class ProjectsWorkbookPage extends BuildPathBasePage {
	...
	public ProjectsWorkbookPage(ListDialogField buildpathList, IWorkbenchPreferenceContainer pageContainer) {
		...
		fProjectsList= new TreeListDialogField(adapter, buttonLabels, new BPListLabelProvider());
...
//				selElement.setAttribute(BPListElement.NATIVE_LIB_PATH, dialog.getNativeLibraryPath());
//				fProjectsList.refresh();
...
//		}
	...
	private BPListElement[] openProjectDialog(BPListElement elem) {
		...
		try {
			...
			ListSelectionDialog dialog= new ListSelectionDialog(getShell(), Arrays.asList(selectArr), new ArrayContentProvider(), new ScriptUILabelProvider(), NewWizardMessages.ProjectsWorkbookPage_chooseProjects_message); 
