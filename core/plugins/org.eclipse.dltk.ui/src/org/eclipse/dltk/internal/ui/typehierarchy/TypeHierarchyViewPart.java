...
public abstract class TypeHierarchyViewPart extends ViewPart implements ITypeHierarchyViewPart, IViewPartInputProvider {
	...
	public TypeHierarchyViewPart() {
		...
		fLinkingEnabled= getPreferenceStore().getBoolean(PreferenceConstants.LINK_TYPEHIERARCHY_TO_EDITOR);
	}
	...
	public IModelElement[] getHistoryEntries() {
		...
		return (IModelElement[]) fInputHistory.toArray(new IModelElement[fInputHistory.size()]);
	}
	...
	public void setInputElement(IModelElement element) {
		...
		if (element != null) {
			if (element instanceof IMember) {
				...
				if (kind != IModelElement.SCRIPT_PROJECT && kind != IModelElement.PROJECT_FRAGMENT) {
					element= null;
					DLTKUIPlugin.logErrorMessage("Invalid type hierarchy input type.");//$NON-NLS-1$
				}
			}
		}	
	...
	private void updateInput(IModelElement inputElement) {
		...
		if (inputElement == null) {	
			...
			try {
				fHierarchyLifeCycle.ensureRefreshedTypeHierarchy(inputElement, DLTKUIPlugin.getActiveWorkbenchWindow());
	...
	public void dispose() {
		...
		if (fPropertyChangeListener != null) {
			DLTKUIPlugin.getDefault().getPreferenceStore().removePropertyChangeListener(fPropertyChangeListener);
...
//		}
	...
	private Control createTypeViewerControl(Composite parent) {
		...
		// Create the viewers
		TypeHierarchyViewer superTypesViewer= new SuperTypeHierarchyViewer(fViewerbook, fHierarchyLifeCycle, this, getPreferenceStore());
		initializeTypesViewer(superTypesViewer, keyListener, IContextMenuConstants.TARGET_ID_SUPERTYPES_VIEW);
		
		TypeHierarchyViewer subTypesViewer= new SubTypeHierarchyViewer(fViewerbook, fHierarchyLifeCycle, this, getPreferenceStore());
		initializeTypesViewer(subTypesViewer, keyListener, IContextMenuConstants.TARGET_ID_SUBTYPES_VIEW);
		
		TypeHierarchyViewer vajViewer= new TraditionalHierarchyViewer(fViewerbook, fHierarchyLifeCycle, this, getPreferenceStore());
	...
	private Control createMethodViewerControl(Composite parent) {
		fMethodsViewer= new MethodsViewer(parent, fHierarchyLifeCycle, this, getPreferenceStore());
...
//	}	
	...
	private void updateTitle() {
		...
		if (fInputElement != null) {
			...
			if (workingSet == null) {
				String[] args= new String[] { viewerTitle, ScriptElementLabels.getDefault().getElementLabel(fInputElement, ScriptElementLabels.ALL_DEFAULT) };
				...
				String[] args= new String[] { viewerTitle, ScriptElementLabels.getDefault().getElementLabel(fInputElement, ScriptElementLabels.ALL_DEFAULT), workingSet.getLabel() };
	...
	public void saveState(IMemento memento) {
		...
		IModelElement selection= (IModelElement)((IStructuredSelection) getCurrentViewer().getSelection()).getFirstElement();
		if (selection != null) {
	...
	private void doRestoreInBackground(final IMemento memento, final IModelElement hierarchyInput, IProgressMonitor monitor) throws ModelException {
...
//		IModelElement elem= (IModelElement)editor.getEditorInput().getAdapter(IModelElement.class);
//		try {
//			IType type= null;
...
//		}
	...
	public void setLinkingEnabled(boolean enabled) {
		...
		getPreferenceStore().setValue(PreferenceConstants.LINK_TYPEHIERARCHY_TO_EDITOR, enabled);
