...
public abstract class OptionsConfigurationBlock {
	...
	private void testIfOptionsComplete(PreferenceKey[] allKeys) {
		for (int i= 0; i < allKeys.length; i++) {
			if (allKeys[i].getStoredValue(fLookupOrder, false, fManager) == null) {
				DLTKUIPlugin.logErrorMessage("preference option missing: " + allKeys[i] + " (" + this.getClass().getName() +')');  //$NON-NLS-1$//$NON-NLS-2$
			}
		}
	}
	
	private int getRebuildCount() {
		return fManager.getWorkingCopy(new DefaultScope().getNode(DLTKUIPlugin.PLUGIN_ID)).getInt(REBUILD_COUNT_KEY, 0);
	}
	
	private void incrementRebuildCount() {
		fRebuildCount++;
		fManager.getWorkingCopy(new DefaultScope().getNode(DLTKUIPlugin.PLUGIN_ID)).putInt(REBUILD_COUNT_KEY, fRebuildCount);
	}
	...
	public void selectOption(PreferenceKey key) {
	...
	protected Button addCheckBox(Composite parent, String label, PreferenceKey key, String[] values, int indent) {
	...
	protected Button addCheckBoxWithLink(Composite parent, String label, PreferenceKey key, String[] values, int indent, int widthHint, SelectionListener listener) {
		ControlData data= new ControlData(key, values);
	...
	protected Combo addInversedComboBox(Composite parent, String label, PreferenceKey key, String[] values, String[] valueLabels, int indent) {
	...
	protected Text addTextField(Composite parent, String label, PreferenceKey key, int indent, int widthHint) {	
	...
	protected void textChanged(Text textControl) {
		PreferenceKey key= (PreferenceKey) textControl.getData();
	 ...
	 */
	protected String getStoredValue(PreferenceKey key) {
		return key.getStoredValue(fLookupOrder, false, fManager);
	}
	 ...
	 */	
	protected abstract void validateSettings(PreferenceKey changedKey, String oldValue, String newValue);
	...
	protected void updateText(Text curr) {
		PreferenceKey key= (PreferenceKey) curr.getData();
	...
	protected Button getCheckBox(PreferenceKey key) {
		for (int i= fCheckBoxes.size() - 1; i >= 0; i--) {
	...
	protected Combo getComboBox(PreferenceKey key) {
		for (int i= fComboBoxes.size() - 1; i >= 0; i--) {
	...
	protected Text getTextControl(PreferenceKey key) {
		for (int i= fTextBoxes.size() - 1; i >= 0; i--) {
	...
	protected Control findControl(PreferenceKey key) {
	...
	protected void setComboEnabled(PreferenceKey key, boolean enabled) {
