...
public final class ProposalSorterHandle {
	...
	private static final String PERFORMANCE_EVENT= DLTKUIPlugin.PLUGIN_ID + "/perf/content_assist_sorters/extensions"; //$NON-NLS-1$
	/**
	...
	private Status createExceptionStatus(InvalidRegistryObjectException x) {
		...
		return new Status(IStatus.INFO, DLTKUIPlugin.getPluginId(), IStatus.OK, disable + " " + reason, x); //$NON-NLS-1$
	}
	...
	private Status createExceptionStatus(CoreException x) {
		...
		return new Status(IStatus.ERROR, DLTKUIPlugin.getPluginId(), IStatus.OK, disable + " " + reason, x); //$NON-NLS-1$
	}
	...
	private Status createExceptionStatus(RuntimeException x) {
		...
		return new Status(IStatus.WARNING, DLTKUIPlugin.getPluginId(), IStatus.OK, disable + " " + reason, x); //$NON-NLS-1$
	}

	private Status createAPIViolationStatus(String operation) {
		String disable= createBlameMessage();
		Object[] args= {operation};
		String reason= Messages.format(ScriptTextMessages.CompletionProposalComputerDescriptor_reason_API, args);
		return new Status(IStatus.WARNING, DLTKUIPlugin.getPluginId(), IStatus.OK, disable + " " + reason, null); //$NON-NLS-1$
	}

	private Status createPerformanceStatus(String operation) {
		String disable= createBlameMessage();
		Object[] args= {operation};
		String reason= Messages.format(ScriptTextMessages.CompletionProposalComputerDescriptor_reason_performance, args);
		return new Status(IStatus.WARNING, DLTKUIPlugin.getPluginId(), IStatus.OK, disable + " " + reason, null); //$NON-NLS-1$
	}

	private String createBlameMessage() {
		Object[] args= { getName(), getId() };
		String disable= Messages.format(ScriptTextMessages.ProposalSorterHandle_blame, args);
		return disable;
	}
