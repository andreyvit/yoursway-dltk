...
public class CreateStepFilterDialog extends StatusDialog {
	...
	static public Filter showCreateStepFilterDialog(Shell parent, Filter[] existingFilters) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		text.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				validateChange();
				if (!filterValid) 
					updateStatus(new StatusInfo(IStatus.ERROR, ScriptDebugPreferencesMessages.CreateStepFilterDialog_4)); 
				else if (isDuplicateFilter(text.getText().trim())) {
					updateStatus(new StatusInfo(IStatus.WARNING, ScriptDebugPreferencesMessages.CreateStepFilterDialog_5)); 
					return;
				} else 
