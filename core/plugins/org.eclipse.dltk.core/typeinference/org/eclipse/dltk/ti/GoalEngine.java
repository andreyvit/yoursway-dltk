...
public class GoalEngine {
	...
	public final static Object RECURSION_RESULT = new Object();
	...
	private final static boolean TRACE_TIME_LIMIT_RECOVERY = Boolean
			.valueOf(
					Platform
							.getDebugOption("org.eclipse.dltk.core/typeInferencing/traceTimeLimit"))
			.booleanValue();
	...
	private int totalGoalsCalculated, totalGoalsRequested,
			totalRecursiveRequests, cacheHits, maxStackSize;
	...
	public Object evaluateGoal(IGoal rootGoal, long timeLimit) {
		if (rootGoal == null)
			return null;
		...
		Object lastObtainedSubgoalResult = answersCache
				.get(rootGoal);
		if (lastObtainedSubgoalResult != null) {
			if (TRACE_GOALS)
				System.out.println("Root goal cache hit: "
						+ rootGoal.toString());
			cacheHits++;
		...
		if (TRACE_GOALS)
			System.out.println("Root goal evaluation: "
					+ rootGoal.toString()
					+ (timeLimit > 0 ? "  time limit: " + timeLimit : ""));

		long endTime = System.currentTimeMillis() + timeLimit;
		GoalEvaluator rootEvaluator = evaluatorFactory
				.createEvaluator(rootGoal);
		if (rootEvaluator == null)
			return null;
		int emptyStackSize = stackSize; // there might already be some elements
										// there
		...
		while (stackSize > emptyStackSize
			...
			IGoal subgoal = goal.produceNextSubgoal(lastFullyEvaluatedGoal,
					lastObtainedSubgoalResult);

			if (subgoal == null) {
				lastObtainedSubgoalResult = goal.produceResult();
				...
				if (isInStack(subgoal)) {
					...
					lastObtainedSubgoalResult = RECURSION_RESULT;
				} else {
					lastObtainedSubgoalResult = (IEvaluatedType) answersCache
							.get(subgoal);
					if (lastObtainedSubgoalResult != null) {
						cacheHits++;
						lastFullyEvaluatedGoal = subgoal;
						...
						GoalEvaluator evaluator = evaluatorFactory
								.createEvaluator(subgoal);
						if (evaluator == null) {
							if (TRACE_GOALS)
								System.out
										.println("No evaluator for goal: "
												+ subgoal);
							lastFullyEvaluatedGoal = subgoal;
						} else {
		...
		while (stackSize > emptyStackSize) {
			...
			if (TRACE_TIME_LIMIT_RECOVERY && subgoal != null)
				System.out.println("Goal ignored due to time limit: "
						+ subgoal.toString());
			lastObtainedSubgoalResult = goal.produceResult();
			lastFullyEvaluatedGoal = goal.getGoal();
		}
