...
//		}
		...
		if (superBinding != null) {
			...
			if (typeBinding.isHierarchyInconsistent()) {
				if (superBinding.problemId() == ProblemReasons.NotFound) {
					...
					this.builder.hierarchy.missingTypes.add(new String(
							superBinding.sourceName)); // note: this could be
														// Map$Entry
					return null;
					...
					if (type instanceof IBinaryType) {
						superclassName = ((IBinaryType) type)
								.getSuperclassName();
						separator = '/';
					} else if (type instanceof ISourceType) {
						superclassName = ((ISourceType) type)
								.getSuperclassName();
						separator = '.';
					...
					if (superclassName != null) { // check whether subclass of
													// Object due to broken
													// hierarchy (as opposed to
													// explicitly extending it)
						int lastSeparator = CharOperation.lastIndexOf(
								separator, superclassName);
						char[] simpleName = lastSeparator == -1 ? superclassName
								: CharOperation.subarray(superclassName,
										lastSeparator + 1,
										superclassName.length);
						if (!CharOperation.equals(simpleName,
			...
			for (int t = this.typeIndex; t >= 0; t--) {
				if (this.typeBindings[t] == superBinding) {
					return this.builder.getHandle(this.typeModels[t],
							superBinding);
				}
			}
		}
	...
	/*
	 * Returns the handles of the super interfaces of the given type. Adds the
	 * simple name to the hierarchy missing types if an interface is not found
	 * (but don't put null in the returned array)
	 */
	private IType[] findSuperInterfaces(IGenericType type,
		...
		if (type instanceof IBinaryType) {
			...
			if (sourceType.getName().length == 0) { // if anonymous type
				if (typeBinding.superInterfaces() != null
						&& typeBinding.superInterfaces().length > 0) {
					superInterfaceNames = new char[][] { sourceType
							.getSuperclassName() };
				} else {
		...
		int bindingIndex = 0;
		int bindingLength = interfaceBindings == null ? 0
				: interfaceBindings.length;
		int length = superInterfaceNames == null ? 0
				: superInterfaceNames.length;
		...
		next: for (int i = 0; i < length; i++) {
			...
			int lastSeparator = CharOperation.lastIndexOf(separator,
					superInterfaceName, 0, end);
			int start = lastSeparator + 1;

			// case of binary inner type -> take the last part
			int lastDollar = CharOperation.lastIndexOf('$', superInterfaceName,
					start);
			if (lastDollar != -1)
				start = lastDollar + 1;

			char[] simpleName = CharOperation.subarray(superInterfaceName,
					start, end);

			if (bindingIndex < bindingLength) {
				ReferenceBinding interfaceBinding = (ReferenceBinding) interfaceBindings[bindingIndex]
						.erasure();

				// ensure that the binding corresponds to the interface defined
				// by the user
				if (CharOperation.equals(simpleName,
						interfaceBinding.sourceName)) {
					bindingIndex++;
					for (int t = this.typeIndex; t >= 0; t--) {
						if (this.typeBindings[t] == interfaceBinding) {
							superinterfaces[index++] = this.builder.getHandle(
									this.typeModels[t], interfaceBinding);
							continue next;
						}
					}
				}
		...
		if (index != length)
			System.arraycopy(superinterfaces, 0,
					superinterfaces = new IType[index], 0, index);
		...
		for (int current = this.typeIndex; current >= 0; current--) {
			...
			if (typeBinding instanceof SourceTypeBinding) {
				...
				if (scope != null) {
					...
					TypeReference superclassRef = typeDeclaration == null ? null
							: typeDeclaration.superclass;
					TypeBinding superclass = superclassRef == null ? null
							: superclassRef.resolvedType;
					if (superclass instanceof ProblemReferenceBinding) {
						superclass = ((ProblemReferenceBinding) superclass)
								.closestMatch();
					}
					if (superclass != null)
					...
					int length;
					ReferenceBinding[] interfaceBindings = typeBinding
							.superInterfaces();
					if (superInterfaces != null
							&& (length = superInterfaces.length) > (interfaceBindings == null ? 0
									: interfaceBindings.length)) { // check for
																	// interfaceBindings
																	// being
																	// null (see
				...
				try {
					...
					// allow subsequent call to superInterfaces() to succeed so
					// that we don't have to catch AbortCompilation everywhere
	...
	private void remember(IGenericType suppliedType,
			ReferenceBinding typeBinding) {
		if (typeBinding == null)
			return;

		if (++this.typeIndex == this.typeModels.length) {
			System.arraycopy(this.typeModels, 0,
					this.typeModels = new IGenericType[this.typeIndex * 2], 0,
					this.typeIndex);
			System
					.arraycopy(
							this.typeBindings,
	...
	private void remember(IType type, ReferenceBinding typeBinding) {
		if (((CompilationUnit) type.getCompilationUnit()).isOpen()) {
			try {
				IGenericType genericType = (IGenericType) ((JavaElement) type)
						.getElementInfo();
			...
			TypeDeclaration typeDeclaration = ((SourceTypeBinding) typeBinding).scope
					.referenceType();
			...
			if (superclass != null) {
				...
				superclassName = typeName == null ? null
			...
			HierarchyType hierarchyType = new HierarchyType(type,
					typeDeclaration.name, typeDeclaration.binding.modifiers,
	 ...
	 */
	private void rememberAllTypes(ModuleDeclaration parsedUnit,
			org.eclipse.jdt.core.ICompilationUnit cu, boolean includeLocalTypes) {
		...
		if (includeLocalTypes && parsedUnit.localTypes != null) {
			...
			for (int i = 0; i < parsedUnit.localTypeCount; i++) {
				...
				IType typeHandle = (IType) factory.createElement(classScope,
						cu, existingElements, knownScopes);
				rememberWithMemberTypes(typeDecl, typeHandle);
			}
		}
	}

	private void rememberWithMemberTypes(TypeDeclaration typeDecl,
			IType typeHandle) {
		...
		if (memberTypes != null) {
			for (int i = 0, length = memberTypes.length; i < length; i++) {
				...
				rememberWithMemberTypes(memberType, typeHandle
						.getType(new String(memberType.name)));
			}
		}
	}
	 ...
	 */
	private void reportHierarchy(IType focus,
		...
		if (focus != null) {
			if (binaryTypeBinding != null) {
				...
				// source type
				Member declaringMember = ((Member) focus)
						.getOuterMostLocalContext();
				if (declaringMember == null) {
					// top level or member type
					char[] fullyQualifiedName = focus.getFullyQualifiedName()
							.toCharArray();
					setFocusType(CharOperation.splitOn('.', fullyQualifiedName));
				} else {
					// anonymous or local type
					if (parsedUnit != null) {
						TypeDeclaration typeDecl = new ASTNodeFinder(parsedUnit)
								.findType(focus);
						if (typeDecl != null) {
		...
		for (int current = this.typeIndex; current >= 0; current--) {
			...
			IType superclass;
			if (typeBinding.isInterface()) { // do not connect interfaces to
												// Object
				superclass = null;
			} else {
				superclass = findSuperClass(suppliedType, typeBinding);
			}
			IType[] superinterfaces = findSuperInterfaces(suppliedType,
					typeBinding);

			this.builder.connect(suppliedType, this.builder.getHandle(
					suppliedType, typeBinding), superclass, superinterfaces);
		}
		...
		if (!this.hasMissingSuperClass && objectIndex > -1) {
			...
			this.builder.connect(objectType, this.builder.getHandle(objectType,
					this.typeBindings[objectIndex]), null, null);
		}
	}
	...
	/**
	 * Resolve the supertypes for the supplied source type. Inform the requestor
	 * of the resolved supertypes using: connect(ISourceType suppliedType,
	 * IGenericType superclass, IGenericType[] superinterfaces)
	 * 
	 * @param suppliedType
	 */
	public void resolve(IGenericType suppliedType) {
		try {
			if (suppliedType.isBinaryType()) {
				BinaryTypeBinding binaryTypeBinding = this.lookupEnvironment
						.cacheBinaryType(
								(IBinaryType) suppliedType,
								false/*
										 * don't need field and method (bug
										 * 125067)
										 */,
								null /* no access restriction */);
				remember(suppliedType, binaryTypeBinding);
				// We still need to add superclasses and superinterfaces
				// bindings (see
				...
				org.eclipse.jdt.core.ICompilationUnit cu = ((SourceTypeElementInfo) suppliedType)
						.getHandle().getCompilationUnit();
				HashSet localTypes = new HashSet();
				localTypes.add(cu.getPath().toString());
				this.superTypesOnly = true;
				resolve(new Openable[] { (Openable) cu }, localTypes, null);
			}
		} catch (AbortCompilation e) { // ignore this exception for now since
										// it typically means we cannot find
										// java.lang.Object
		} finally {
			reset();
		}
	 ...
	 * connect(ISourceType suppliedType, IGenericType superclass, IGenericType[]
	 * superinterfaces)
	 * 
	 * Also inform the requestor of the supertypes of each additional requested
	 * super type which is also a source type instead of a binary type.
	 * 
	 * @param openables
	 * @param localTypes
	 * @param monitor
	...
	public void resolve(Openable[] openables, HashSet localTypes,
		...
		try {
			...
			// build type bindings
			Parser parser = new Parser(this.lookupEnvironment.problemReporter,
					true);
			for (int i = 0; i < openablesLength; i++) {
				...
				if (openable instanceof org.eclipse.jdt.core.ICompilationUnit) {
					...
					// contains a potential subtype as a local or anonymous
					// type?
					...
					if (cu.isOpen()) {
						...
						CompilationResult result = new CompilationResult(
								((ICompilationUnit) cu).getFileName(), i,
								openablesLength,
						...
						ICompilationUnit sourceUnit = this.builder
								.createCompilationUnitFromPath(openable, file);

						CompilationResult unitResult = new CompilationResult(
								sourceUnit, i, openablesLength,
								this.options.maxProblemsPerUnit);
					...
					if (classFile.isOpen()) {
						...
						try {
							binaryType = (IBinaryType) ((JavaElement) type)
									.getElementInfo();
						} catch (JavaModelException e) {
						...
						if (classFile.getPackageFragmentRoot().isArchive()) {
							binaryType = this.builder
									.createInfoFromClassFileInJar(classFile);
						} else {
							IResource file = classFile.getResource();
							binaryType = this.builder.createInfoFromClassFile(
									classFile, file);
						}
					}
					if (binaryType != null) {
			...
			for (int i = 0; i < unitsIndex; i++) {
				...
				if (parsedUnit != null) {
					try {
						...
						if (containsLocalType) { // NB: no-op if method
													// bodies have been already
													// parsed
							parser.getMethodBodies(parsedUnit);
						}
						// complete type bindings and build fields and methods
						// only for local types
						// (in this case the constructor is needed when
						// resolving local types)
						// (see
						// https://bugs.eclipse.org/bugs/show_bug.cgi?id=145333)
						this.lookupEnvironment.completeTypeBindings(parsedUnit,
								containsLocalType);
					} catch (AbortCompilation e) {
						// classpath problem for this type: don't try to resolve
						// (see
			...
			// if no potential subtype was a real subtype of the binary focus
			// type, no need to go further
			...
			if (focusBinaryBinding == null && focus != null && focus.isBinary()) {
				...
				focusBinaryBinding = this.lookupEnvironment
						.getCachedType(CharOperation.splitOn('.',
								fullyQualifiedName));
		...
		} catch (ClassCastException e) { // work-around for 1GF5W1S - can
											// happen in case duplicates are fed
											// to the hierarchy with binaries
											// hiding sources
		} catch (AbortCompilation e) { // ignore this exception for now since
										// it typically means we cannot find
										// java.lang.Object
	...
	public boolean subOrSuperOfFocus(ReferenceBinding typeBinding) {
		...
		try {
			...
			if (!this.superTypesOnly
					&& this.subTypeOfType(typeBinding, this.focusType))
				return true;
		} catch (AbortCompilation e) {
	...
	private boolean subTypeOfType(ReferenceBinding subType,
			ReferenceBinding typeBinding) {
		...
		if (superInterfaces != null) {
			for (int i = 0, length = superInterfaces.length; i < length; i++) {
				ReferenceBinding superInterface = (ReferenceBinding) superInterfaces[i]
						.erasure();
