...
/**
 * Common protocol for all elements provided by the script model.
 * Script model elements are exposed to clients as handles to the actual underlying element.
 * The model may hand out any number of handles for each element. Handles
 ...
 * a <code>ModelException</code> when an underlying element is missing.
 * <code>ModelException.isDoesNotExist</code> can be used to recognize
...
public interface IModelElement extends IAdaptable {
	...
	/**
	 * Constant representing a script model (workspace level object).
	 * An element with this type can be safely cast to <code>IModel</code>.
	 */
	int SCRIPT_MODEL = 1;
	...
	/**
	 * Constant representing a project fragment.
	 * An element with this type can be safely cast to <code>IProjectFragment</code>.
	 */
	...
	/**
	 * Constant representing a binary module.
	 * An element with this type can be safely cast to <code>IBinaryModule</code>.
	 */
	int BINARY_MODULE = 6;
	 ...
	 * A model element with this type can be safely cast to <code>IPackageDeclaration</code>.
	 */
	...
	/**
	 * Returns whether this element is read-only. An element is read-only
	 * if its structure cannot be modified by the model. 
	 * <p>
	 * Note this is different from IResource.isReadOnly(). For example, .zip
	 * files are read-only as the model doesn't know how to add/remove 
	 ...
	 * to navigated to any existing model element from the root of the model
	 * along a chain of existing model elements. On the other hand, working
	...
	/**
	 * Returns the Script project this element is contained in,
	 * or <code>null</code> if this element is not contained in any script project
	 * (for instance, the <code>IScriptModel</code> is not contained in any Script 
	 * project).
	 * This is a handle-only method.
	 *
	 ...
	 * soure module that could not be parsed, <code>false</code> is returned.
	 ...
	 * @exception ScriptModelException if this element does not exist or if an
	 *		exception occurs while accessing its corresponding resource
