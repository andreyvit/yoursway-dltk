 ...
 * <p>
 * Signatures obtained from parsing source files differ subtly
 ...
 * <li><code>"Ljava.lang.String;"</code> denotes
 * <code>java.lang.String</code> in compiled code</li>
 * <li><code>"QString;"</code> denotes <code>String</code> in source code</li>
 * <li><code>"Qjava.lang.String;"</code> denotes
 * <code>java.lang.String</code> in source code</li>
 * <li><code>"[QString;"</code> denotes <code>String[]</code> in source
 * code</li>
 * <li><code>"QMap&lt;QString;*&gt;;"</code> denotes
 * <code>Map&lt;String,?&gt;</code> in source code</li>
 * <li><code>"Qjava.util.List&ltTV;&gt;;"</code> denotes
 * <code>java.util.List&lt;V&gt;</code> in source code</li>
 * <li><code>"&ltE;&gt;Ljava.util.List;"</code> denotes
 * <code>&lt;E&gt;java.util.List</code> in source code</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <li><code>"([Ljava.lang.String;)V"</code> denotes
 * <code>void foo(java.lang.String[])</code> in compiled code</li>
 * <li><code>"(QString;)QObject;"</code> denotes
 * <code>Object foo(String)</code> in source code</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <li><code>"X:QReader;"</code> denotes <code>X extends Reader</code> in
 * source code</li>
 * <li><code>"X:QReader;:QSerializable;"</code> denotes
 * <code>X extends Reader & Serializable</code> in source code</li>
 * </ul>
 * </p>
 * <p>
...
public final class Signature {
	...
	/**
	 * Character constant indicating an unbound wildcard type argument in a
	 ...
	 */
	public static final char C_TYPE = 'L';
	...
	private static int checkName(char[] name, char[] typeName, int pos,
			int length) {
	...
	public static char[] createMethodSignature(char[][] parameterTypes) {
		...
		char[] result = new char[1 + parameterLength + 1];
	...
	public static String createTypeSignature(char[] typeName) {
		return new String(createCharArrayTypeSignature(typeName));
	}
	...
	public static char[] createCharArrayTypeSignature(char[] typeName) {
		...
		int pos = encodeTypeSignature(typeName, 0, length, buffer);
	...
	private static int encodeQualifiedName(char[] typeName, int pos,
			int length, StringBuffer buffer) {
		int count = 0;
		...
		nameLoop: while (pos < length) {
			...
			default:
				if (currentChar == ' '
						|| ScannerHelper.isWhitespace(currentChar)) {
					if (lastAppendedChar == C_DOT) { // allow spaces after a
														// dot
						pos = consumeWhitespace(typeName, pos, length) - 1; // will
																			// be
																			// incremented
						break;
					}
					// allow spaces before a dot
					int checkPos = checkNextChar(typeName, '.', pos, length,
							true);
					if (checkPos > 0) {
						buffer.append(C_DOT); // process dot immediately to
												// avoid one iteration
						lastAppendedChar = C_DOT;
						count++;
						pos = checkPos;
	...
	private static int encodeArrayDimension(char[] typeName, int pos,
			int length, StringBuffer buffer) {
		int checkPos;
		while (pos < length
				&& (checkPos = checkNextChar(typeName, '[', pos, length, true)) > 0) {
	...
	private static int checkNextChar(char[] typeName, char expectedChar,
			int pos, int length, boolean isOptional) {
		pos = consumeWhitespace(typeName, pos, length);
	...
	private static int encodeTypeSignature(char[] typeName, int start,
			int length, StringBuffer buffer) {
		int pos = start;
		pos = consumeWhitespace(typeName, pos, length);
		if (pos >= length)
		...
		case '?':
			...
			if (checkPos > 0) {
				...
				pos = encodeTypeSignature(typeName, checkPos, 
						length, buffer);
				return pos;
			}
		...
		while (true) { // loop on qualifiedName[<args>][.qualifiedName[<args>]*
			...
			if (checkPos > 0) {
				...
				pos = encodeTypeSignature(typeName, checkPos, 
						length, buffer);
				while ((checkPos = checkNextChar(typeName, ',', pos, length,
						true)) > 0) {
					pos = encodeTypeSignature(typeName, checkPos, 
							length, buffer);
				}
	 ...
	 *         {@link #BASE_TYPE_SIGNATURE}, or
	 *         {@link #TYPE_VARIABLE_SIGNATURE}, or 
	 *         {@link #WILDCARD_TYPE_SIGNATURE} or
	...
	public static char[] getTypeErasure(char[] parameterizedTypeSignature)
			throws IllegalArgumentException {
		int end = CharOperation.indexOf(C_GENERIC_START,
		...
		for (int idx = end; idx < length; idx++) {
			...
			case C_GENERIC_START:
				if (deep == 0) {
					int size = idx - start;
					System.arraycopy(parameterizedTypeSignature, start, result,
							pos, size);
					end = idx;
					pos += size;
				}
		...
		System.arraycopy(parameterizedTypeSignature, start, resized, pos,
				length - start);
		return resized;
	}
	...
	public static String getTypeErasure(String parameterizedTypeSignature)
			throws IllegalArgumentException {
		return new String(getTypeErasure(parameterizedTypeSignature
				.toCharArray()));
	}
	...
	public static String getTypeVariable(String formalTypeParameterSignature)
			throws IllegalArgumentException {
		return new String(getTypeVariable(formalTypeParameterSignature
				.toCharArray()));
	}
	 ...
	 *
	 */
	public static char[][] getTypeParameterBounds(
		...
		int p2 = CharOperation.indexOf(C_COLON, formalTypeParameterSignature,
				p1 + 1);
		char[] classBound;
		if (p2 < 0) {
			// no interface bounds
			classBound = CharOperation.subarray(formalTypeParameterSignature,
					p1 + 1, formalTypeParameterSignature.length);
		...
		char[][] interfaceBounds = CharOperation.splitOn(C_COLON,
				formalTypeParameterSignature, p2 + 1,
	 ...
	 *
	 */
	public static String[] getTypeParameterBounds(
			String formalTypeParameterSignature)
			throws IllegalArgumentException {
		char[][] bounds = getTypeParameterBounds(formalTypeParameterSignature
				.toCharArray());
	...
	public static char[] getSignatureSimpleName(char[] typeSignature) {
		...
		if (dotCount > 0) {
			for (int i = 0; i < qualifiedType.length; i++) {
				...
				if (dotCount <= 0) {
					return CharOperation.subarray(qualifiedType, i + 1,
	...
	public static String getSignatureSimpleName(String typeSignature) {
		return new String(getSignatureSimpleName(typeSignature == null ? null
				: typeSignature.toCharArray()));
	}
	...
	public static char[] getSimpleName(char[] name) {
		...
		appendArgumentSimpleNames(name, lastGenericStart, lastGenericEnd,
				buffer);
		buffer.append(name, lastGenericEnd + 1, length - lastGenericEnd - 1); // copy
																				// trailing
																				// portion,
																				// may
																				// contain
	...
	public static String getSimpleName(String name) {
		...
		appendArgumentSimpleNames(nameChars, lastGenericStart, lastGenericEnd,
				buffer);
		buffer.append(nameChars, lastGenericEnd + 1, length - lastGenericEnd
				- 1); // copy trailing portion, may contain dimensions
		return buffer.toString();
	}

	private static void appendSimpleName(char[] name, int start, int end,
			StringBuffer buffer) {
		int lastDot = -1, lastGenericStart = -1, lastGenericEnd = -1;
		int depth = 0;
		...
		if (lastGenericStart >= 0) {
			appendArgumentSimpleNames(name, lastGenericStart, lastGenericEnd,
					buffer);
			buffer.append(name, lastGenericEnd + 1, end - lastGenericEnd); // copy
																			// trailing
																			// portion,
																			// may
																			// contain
	...
	private static void appendArgumentSimpleNames(char[] name, int start,
	 ...
	 *            <code>null</code> if no parameter names are to be included;
	 *            if supplied, the number of parameter names must match that of
	 *            the method signature
	 * @param fullyQualifyTypeNames
	 *            <code>true</code> if type names should be fully qualified,
	 *            and <code>false</code> to use only simple names
	 ...
	 *            <code>true</code> if the return type is to be included
	 * @param isVargArgs
	 *            <code>true</code> if the last argument should be displayed
	 *            as a variable argument, <code>false</code> otherwise.
	 * @return the char array representation of the method signature
	 * 
	 *
	 */
	 ...
	 */
	private static int appendTypeSignature(char[] string, int start,
			boolean fullyQualifyTypeNames, StringBuffer buffer) {
		return appendTypeSignature(string, start, fullyQualifyTypeNames,
				buffer, false);
	}
	/**
	 ...
	 *            variable argument, <code>false</code> otherwise. In this
	 *            case, the type must be an array type
	 * @return the 0-based character index of the last character
	 * @exception IllegalArgumentException
	 *                if this is not a type signature, or if isVarArgs is
	 *                <code>true</code>, and the type is not an array type
	 *                signature.
	 * @see Util#scanTypeSignature(char[], int)
	 */
	private static int appendTypeSignature(char[] string, int start,
		...
		if (isVarArgs) {
			switch (c) {
			case C_ARRAY:
				return appendArrayTypeSignature(string, start,
						fullyQualifyTypeNames, buffer, true);
			case C_TYPE:
			case C_TYPE_VARIABLE:
			case C_VOID:
			case C_STAR:
			...
			case C_CAPTURE:
			default:
				throw new IllegalArgumentException(); // a var args is an
														// array type
			}
		} else {
			switch (c) {
			...
			case C_EXTENDS:
			case C_SUPER:
				return appendTypeArgumentSignature(string, start,
						fullyQualifyTypeNames, buffer);
			default:
				throw new IllegalArgumentException();
			}
	 ...
	 */
	private static int appendArrayTypeSignature(char[] string, int start,
			boolean fullyQualifyTypeNames, StringBuffer buffer) {
		return appendArrayTypeSignature(string, start, fullyQualifyTypeNames,
				buffer, false);
	}
	 ...
	 * @see Util#scanArrayTypeSignature(char[], int)
	 */
	private static int appendCaptureTypeSignature(char[] string, int start,
			boolean fullyQualifyTypeNames, StringBuffer buffer) {
		...
		return appendTypeArgumentSignature(string, start + 1,
				fullyQualifyTypeNames, buffer);
	}
	...
	private static int appendArrayTypeSignature(char[] string, int start,
		...
		int e = appendTypeSignature(string, index, fullyQualifyTypeNames,
				buffer);
	 ...
	 */
	private static int appendClassTypeSignature(char[] string, int start,
			boolean fullyQualifyTypeNames, StringBuffer buffer) {
		...
		while (true) {
			...
			case C_SEMICOLON:
				...
				return p;
			case C_GENERIC_START:
				int e = appendTypeArgumentSignatures(string, p,
						fullyQualifyTypeNames, buffer);
	 ...
	 */
	private static int appendTypeArgumentSignatures(char[] string, int start,
			boolean fullyQualifyTypeNames, StringBuffer buffer) {
		// need a minimum 2 char "<>"
		if (start >= string.length - 1) {
			throw new IllegalArgumentException();
		...
		while (true) {
			...
			int e = appendTypeArgumentSignature(string, p,
					fullyQualifyTypeNames, buffer);
			count++;
			p = e + 1;
		}
	 ...
	 */
	private static int appendTypeArgumentSignature(char[] string, int start,
			boolean fullyQualifyTypeNames, StringBuffer buffer) {
		// need a minimum 1 char
		if (start >= string.length) {
			throw new IllegalArgumentException();
		...
		case C_EXTENDS:
			...
			return appendTypeSignature(string, start + 1,
					fullyQualifyTypeNames, buffer);
		case C_SUPER:
