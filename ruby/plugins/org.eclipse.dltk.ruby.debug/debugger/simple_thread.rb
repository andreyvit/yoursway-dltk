...
module XoredDebugger
    ...
    class RubyThread < CommandHandler
		# Events
        def created
			log_manager.puts("Thread 'created' event")
            io_manager.send('init', init('app_test_id', @key, thread_label, @script))
        end
        
        def terminated
            log_manager.puts('Thread terminated event!')
            unless @f_term
                sent_stopped
            end         
        end
		...
		def terminate
            log_manager.puts('Terminating thread... ' + @thread.inspect)
        ...
        def trace(event, file, line, id, binding, klass)       
            ...
            case event
                when 'line'
                    ...
                    @stack_manager.stack.update(binding, @file, line, where)

                    br_break = breakpoint_manager.line_break?(@stack_manager.stack, @file, line)
                    log_manager.puts("Breakpoint test result: line : #{line}, break: #{br_break}")
                    ...
                    if (@io.has_data? or
                        ...
                        log_manager.puts("==>> Line ##{line} from #{@file} <<==")
                    ...
                    #capturer.enable
                when 'call'
                    @stack_manager.stack.push(binding, @file, line, where)
