...
public class PythonAutoEditStrategy extends DefaultIndentLineAutoEditStrategy {
	 ...
	 * possible prefereces:
	 * 
	 * general indent on/off
	 * smart tab key behaviour
	 * autoclose "strings", 'strings'
	 * autoclose {brackets}, [], ()
	 * smart paste
	 */
	...
	/**
	 * Find line with number <=line, that is not empty and is
	 * not a comment line starting with #
	 * 
	 * @param d
	...
	/**
	 * Searchs an pair from offset, forward of backwards. Can skip
	 * strings and comments (uses python partitioning). Doesn't goes more 
	 * maxCharsAway chars away from offset.
	 * @param d
	 * @param offset
	 * @param forward
	 * @param opening
	...
	private int searchPair(IDocument d, int offset, boolean forward,
		...
		if (forward) {
			while (i < d.getLength()) {
				ITypedRegion region = TextUtilities.getPartition(d, fPartitioning,
						i, true);
				if (region.getType() == IPythonPartitions.PYTHON_COMMENT && skipCommentLines) {
					i = region.getOffset() + region.getLength();
					continue;
				}
				if (region.getType() == IPythonPartitions.PYTHON_STRING && skipStrings) {
					i = region.getOffset() + region.getLength();
					continue;
				}
				char c = d.getChar(i);
	...
	/**
	 * Finds line < <b>line</b>, such the indent of it is less than
	 * of <b>line</b> 
	 * @param d the document to search in
	 * @param line appropriate line number
	 ...
	 * except \t has length 1, \t has length 8.
	...
	private static void removeStuff(Document document) {
		document.setDocumentPartitioner(IPythonPartitions.PYTHON_PARTITIONING, null);
	}
	 ...
	 * @param d the processed document
	 * @param line line, indenting of which we wanna know
	 * @param newLine if <b>line</b> is new line, so we have pressed enter-key,
	 * and need indentation for new line 
	 * @param offset position, where we have jumped to new line
	 * @return correct indent for line, or null if we should use "default" indent.
	 * So calling side should determine, change indent or not, or may be use 
	...
	private String calcLineIndent(IDocument d, int line, boolean newLine, int offset)
		...
		ITypedRegion region = TextUtilities.getPartition(d, fPartitioning, d.getLineOffset(lastNonEmptyLine), true);
		...
		try {//If the current line begins with a keyword that lines up with "try"
			String curLine = "";			
			if  (lastNonEmptyLine == line - 1 && newLine) {
			...
			if (curLine.startsWith("except") ||
					curLine.startsWith("finally")) {
				int lnum = line - 1;
				
				while (lnum >= 0) {
					String temp =  getDocumentLine(d, lnum).trim();
					if (temp.startsWith("try") ||
							temp.startsWith("except")) {
...
//				System.out.println ("No matching 'try'!");
			...
			if (curLine.startsWith("elif") ||
					curLine.startsWith("else")) {
...
//		If the previous line was a stop-execution statement...
		...
		if (Pattern.matches(regex, plineTrimmed)){
			...
			int sline = findIndentStart (d, lastNonEmptyLine);			
			String reqIndent = getLineIndent (d, sline);
			if (newLine || isDocumentEnd || (getPhysicalLength(getLineIndent(d, line)) > getPhysicalLength(reqIndent))) {
				return reqIndent;
			}
			//trust the user			
			return null;
	...
	/**
	 * If we have pressed ":" for example, than we need to reindent line.
	 * This function changes document and sets correct indent for current line.
	 * @param d
	 * @param c
	 */
...
//					check partition
	 ...
	 * jump to correct line start position.
	 * @param d
	 * @param c
	 * @return
	 */
	private boolean smartIndentJump(IDocument d, DocumentCommand c) {
		...
		try {
			...
			if (resultIndent == null) { //we should save indentation
				...
				if (curLine.trim().length() > 0) //if indentation is "real", use it
					resultIndent = currentIndent;
				else {
					//get current block level
					int pl = getLastNonEmptyLine(d, line - 1); //find last code line
					if (pl >= 0) {
	...
	private void smartPaste(IDocument d, DocumentCommand c) {
		/*
		 * We are creating temp document, inserting text as is, and
		 * then sequentially calling calcLineIndent for each new line.
		 */
		try {
...
//							getRegionType(d, c.offset - 1) == IPythonPartitions.PYTHON_STRING)
